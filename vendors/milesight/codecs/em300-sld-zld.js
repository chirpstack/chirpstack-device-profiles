...
function setReportInterval(report_interval) {
    ...
    var buffer = new EncoderBuffer(4);
...
function setHistoryEnable(history_enable) {
    ...
    var buffer = new EncoderBuffer(3);
    buffer.writeUInt8(0xff);
    buffer.writeUInt8(0x68);
    buffer.writeUInt8(getEncoderValue(enable_map, history_enable));
    return buffer.toBytes();
}
...
function fetchHistory(fetch_history) {
    ...
    if ("end_time" in fetch_history || end_time === 0) {
        buffer = new EncoderBuffer(6);
        buffer.writeUInt8(0xfd);
        buffer.writeUInt8(0x6b);
        buffer.writeUInt32LE(start_time);
    } else {
        buffer = new EncoderBuffer(10);
        buffer.writeUInt8(0xfd);
...
function stopTransmit(stop_transmit) {
    ...
    if (getEncoderValue(yes_no_map, stop_transmit) === 0) {
        return [];
    }
...
function clearHistory(clear_history) {
    ...
    if (getEncoderValue(yes_no_map, clear_history) === 0) {
        return [];
    }
...
function setRetransmitEnable(retransmit_enable) {
    ...
    buffer.writeUInt8(getEncoderValue(enable_map, retransmit_enable));
    return buffer.toBytes();
}
...
function setRetransmitInterval(retransmit_interval) {
    ...
    var buffer = new EncoderBuffer(5);
    buffer.writeUInt8(0xff);
...
function setTemperatureAlarmConfig(temperature_alarm_config) {
    ...
    var condition_map = { 0: "disable", 1: "below", 2: "above", 3: "between", 4: "outside" };
    var condition_values = getEncoderValues(condition_map);
    if (condition_values.indexOf(condition) === -1) {
        throw new Error("condition must be one of " + condition_values.join(", "));
    }

    var data = getEncoderValue(condition_map, condition) | (1 << 3);

    var buffer = new EncoderBuffer(11);
    buffer.writeUInt8(0xff);
...
function setLeakageAlarm(leakage_alarm_config) {
    ...
    var data = getEncoderValue(enable_map, enable) | (2 << 3);
...
function setTemperatureCalibration(temperature_calibration_settings) {
    ...
    var data = (getEncoderValue(enable_map, enable) << 7) | 0x00;
    var buffer = new EncoderBuffer(5);
    buffer.writeUInt8(0xff);
...
function setHumidityCalibration(humidity_calibration_settings) {
    ...
    var data = (getEncoderValue(enable_map, enable) << 7) | 0x01;
    var buffer = new EncoderBuffer(5);
    buffer.writeUInt8(0xff);
    buffer.writeUInt8(0xea);
    buffer.writeUInt8(data);
...
EncoderBuffer.prototype._write = function (value, byteLength, isLittleEndian) {
    var offset = 0;
...
EncoderBuffer.prototype.writeUInt8 = function (value) {
    this._write(value, 1, true);
    this.offset += 1;
};

EncoderBuffer.prototype.writeInt8 = function (value) {
    this._write(value < 0 ? value + 0x100 : value, 1, true);
    this.offset += 1;
};

EncoderBuffer.prototype.writeUInt16LE = function (value) {
    this._write(value, 2, true);
    this.offset += 2;
};

EncoderBuffer.prototype.writeInt16LE = function (value) {
    this._write(value < 0 ? value + 0x10000 : value, 2, true);
    this.offset += 2;
};

EncoderBuffer.prototype.writeUInt32LE = function (value) {
    this._write(value, 4, true);
    this.offset += 4;
};

EncoderBuffer.prototype.writeInt32LE = function (value) {
    this._write(value < 0 ? value + 0x100000000 : value, 4, true);
    this.offset += 4;
};

EncoderBuffer.prototype.writeD2DCommand = function (value, defaultValue) {
...
EncoderBuffer.prototype.toBytes = function () {
    return this.buffer;
};
